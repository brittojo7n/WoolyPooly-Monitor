...
function buildHourlyGraph(graph, now = Date.now()) {
  ...
  const firstHour = currentHour - (WINDOW_HOURS - 1) * HOUR_MS;
  ...
  for (let i = 0; i < WINDOW_HOURS; i++) {
    ...
    const current = (i === WINDOW_HOURS - 1);
    hours.push({
...
function estimateRollingDay(graph, now = Date.now(), options = {}) {
  ...
  const completed = hourlyGraph.slice(0, hourlyGraph.length - 1);
  const reportedHours = completed.filter(b => b.status === 'reported').length;
  const observed24h = hourlyGraph.reduce((total, b) => total + (b.amount == null ? 0 : b.amount), 0);
  ...
  const base = {
    ...
    observed24h, reportedHours, missingHours: WINDOW_HOURS - hourlyGraph.filter(b => b.amount != null).length,
  ...
  const currentIndex = hourlyGraph.length - 1;
  const hasCurrentBucket = hourlyGraph[currentIndex] && hourlyGraph[currentIndex].status === 'partial';
