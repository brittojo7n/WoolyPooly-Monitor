...
function processMetrics(rawData, coinId, wallet, estimate) {
  ...
  const acct = accountStats || {};
  const perf = acct.perfomance || acct.performance || stats.perfomance || stats.performance || {};
  const profitGraph = estimator.attachHashrate(
    stats.minerProfitGraph,
    estimator.extractSeries(perf.pplns, now),
    estimator.extractSeries(perf.solo, now)
  );
