...
function processMetrics(rawData, coinId, wallet, history) {
  ...
  const est = (history && history.estimate) || {};
  ...
  return {
    ...
    estimated: {
      ...
      perWeek: estWeek,
      perMonth: estMonth,
      productiveBuckets: Math.floor(num(est.productiveBuckets)),
      sessionActive: !!est.sessionActive,
      sessionId: est.sessionId != null ? String(est.sessionId) : null,
      sessionStartT: est.sessionStartT != null ? num(est.sessionStartT) : null,
      available: estAvail
    },
    payout: {
      threshold: minPay,
      ...
      remaining,
      ratePerHour,
      rateSource: 'Estimated rate from productive buckets',
      etaHours
    },
    analytics: {
