...
function estimateState(coin, wallet, now) {
  const key = feedKey(coin, wallet);
  const stored = history.getBuckets(key);
  return {
    buckets: stored.length,
    oldestT: stored.length ? stored[0].t : null,
    newestT: stored.length ? stored[stored.length - 1].t : null,
    estimate: estimator.estimateFromBuckets(stored, now)
  };
}

function updateEstimate(coin, wallet, accountStats, now) {
  const key = feedKey(coin, wallet);
  if (accountStats && accountStats.stats && Array.isArray(accountStats.stats.minerProfitGraph)) {
    const candidates = [];
    for (const raw of accountStats.stats.minerProfitGraph) {
      const b = estimator.normalizeBucket(raw, now);
      if (b) candidates.push(b);
    }
    history.recordBuckets(key, candidates);
  }
  return estimator.estimateFromBuckets(history.getBuckets(key), now);
}
...
function processFeedSnapshot(snapshot, coin, wallet) {
  ...
  const hist = {
    estimate: updateEstimate(coin, wallet, accountOk ? snapshot.accountResult.data : null, now)
  };
...
async function handleDebugHistory(req, res, searchParams) {
  try {
    const { coin, wallet } = await resolveRequestConfig(searchParams);
    const now = Date.now();
    const st = estimateState(coin, wallet, now);
    sendJson(res, 200, {
      key: coin + '|' + redactWallet(wallet),
      wallet: redactWallet(wallet),
      coin,
      bucketsArchived: st.buckets,
      oldestBucket: st.oldestT,
      newestBucket: st.newestT,
      sessionId: st.estimate.sessionId,
      sessionActive: st.estimate.sessionActive,
      windowBuckets: st.estimate.windowBuckets,
      windowIncome: st.estimate.windowIncome,
      perHour: st.estimate.perHour
    });
  } catch (err) {
    sendError(res, err);
  }
